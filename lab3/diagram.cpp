...
void TDiagram::PrintError(int stringNumber) {
	...
	exit(2);	// при ошибке - сразу выходим, т.к. наш трансл€тор работает до первой ошибки
}


void TDiagram::Prog() {					// аксиома
	int uk = scan->getUK();				// проверка услови€ цикла или if требует дополнительного сканировани€
	int str = scan->getStringNumber();	// 2 переменные, чтобы можно было вернуть указатель и номер строки до сканировани€
	TypeLex lex;						// локальна€ лексема
...
void TDiagram::GlobalVar() {			// Ќ≈ ƒќƒ≈ЋјЌќ!!!!!!!
	
	TypeLex lex;
	int lexType = scan->scaner(lex);
	// не пон€тно до куда провер€ть
